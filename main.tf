...
resource "newrelic_notification_channel" "notify-channel" {
  ...
  destination_id = each.value.name=="slack-notification-channel"?"${data.newrelic_notification_destination.slack_destination.id}":"${newrelic_notification_destination.email_destination.id}"
